...
class PathResolver:
    ...
    def _next_available_name(self, preview: RenamePreview) -> str:
        ...
        while True:
            ...
            if not (Path(preview.destination_root) / preview.target_subpath / candidate).exists():
                return candidate
            idx += 1
    ...
    def _can_replace_existing(self, preview: RenamePreview):
        ...
        try:
            ...
            target_item = db_session.query(MediaItem).filter(MediaItem.current_path == target_path).first()
            if not target_item:
                return None
