...
from sqlalchemy import or_, and_, desc, select, func, union_all, literal, cast, String, Integer, Float, case, inspect

from ..db.models import CustomList, CustomListItem, ItemStatus, ItemType, MediaItem, VirtualMediaState, UserSetting
...
class LibraryTabService:
    ...
    def get_library_tab_page(
        ...
        counts = self._build_media_tab_counts()
    ...
    def get_grouped_library(self, requested_tabs: Optional[set[str]] = None) -> LibraryGroupedDTO:
        from ..db.models import CustomList, CustomListItem, VirtualMediaState
        from ..db.models.metadata import MediaMatch, OMDBCache, TMDBCache
        ...
        if candidate_tmdb_ids:
            for tmdb_id in candidate_tmdb_ids:
                for media_type in ("movie", "tv"):
                    cache = _pick_tmdb_cache(self.db, tmdb_id, media_type, preferred_languages)
                    if cache and isinstance(cache.raw_data, dict):
                        raw_cache_map[(media_type, tmdb_id)] = cache.raw_data

        omdb_map = {}
        imdb_ids = {
            raw_data.get("external_ids", {}).get("imdb_id")
